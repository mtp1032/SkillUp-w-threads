...
function sig:isValid( signal )
    ...
    if type( signal ) ~= "number" then return false end
    if signal < SIG_NONE then return false end
    if signal > SIG_LAST then return false end
...
function sig:sendSignal( thread_h, signal )
    ...
    local isValid = sig:isValid( signal )
